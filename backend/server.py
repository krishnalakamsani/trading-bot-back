...
# ------------------ MDS metrics endpoint ------------------
@api_router.post("/strategy/mds/metrics")
async def update_mds_metrics(payload: dict):
    """Update latest MDS metrics used by the bot (any subset of accepted keys)."""
    result = await bot_service.update_mds_metrics(payload)
    if result.get('status') == 'success':
        return JSONResponse(content=result)
    else:
        raise HTTPException(status_code=400, detail=result.get('message', 'Failed to update metrics'))
