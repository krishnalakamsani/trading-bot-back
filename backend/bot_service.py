...
async def update_config_values(updates: dict) -> dict:
    ...
    if updates.get('mds_confirm_needed') is not None:
        try:
            config['mds_confirm_needed'] = int(updates['mds_confirm_needed'])
            updated_fields.append('mds_confirm_needed')
            logger.info(f"[CONFIG] MDS confirm needed set to: {config['mds_confirm_needed']}")
        except Exception as e:
            logger.warning(f"[CONFIG] Invalid mds_confirm_needed: {e}")
    ...
    # Update indicator parameters if provided (MACD only)
    indicator_params = {
        'macd_fast': int,
        'macd_slow': int,
        'macd_signal': int,
...
async def update_mds_metrics(metrics: dict) -> dict:
    """Update latest MDS metrics that the strategy will use.

    Expected keys (any subset):
      - mds_ready (bool)
      - mds_is_choppy (bool)
      - mds_direction ("CE" / "PE" / "NONE")
      - mds_score (float)
      - mds_slope (float)
      - mds_confirm_count (int)
      - mds_slow_mom (float)
    """
    logger.info(f"[MDS] Received metrics: {list(metrics.keys())}")
    updated = []
    # Map known keys into bot_state
    if 'mds_ready' in metrics:
        bot_state['mds_ready'] = bool(metrics['mds_ready'])
        updated.append('mds_ready')
    if 'mds_is_choppy' in metrics:
        bot_state['mds_is_choppy'] = bool(metrics['mds_is_choppy'])
        updated.append('mds_is_choppy')
    if 'mds_direction' in metrics:
        bot_state['mds_direction'] = str(metrics['mds_direction'])
        updated.append('mds_direction')
    if 'mds_score' in metrics:
        try:
            bot_state['mds_score'] = float(metrics['mds_score'])
            updated.append('mds_score')
        except Exception:
            logger.warning('[MDS] Invalid mds_score')
    if 'mds_slope' in metrics:
        try:
            bot_state['mds_slope'] = float(metrics['mds_slope'])
            updated.append('mds_slope')
        except Exception:
            logger.warning('[MDS] Invalid mds_slope')
    if 'mds_confirm_count' in metrics:
        try:
            bot_state['mds_confirm_count'] = int(metrics['mds_confirm_count'])
            updated.append('mds_confirm_count')
        except Exception:
            logger.warning('[MDS] Invalid mds_confirm_count')
    if 'mds_slow_mom' in metrics:
        try:
            bot_state['mds_slow_mom'] = float(metrics['mds_slow_mom'])
            updated.append('mds_slow_mom')
        except Exception:
            logger.warning('[MDS] Invalid mds_slow_mom')

    # If external metrics provide a confirm count, push it into runner state if possible
    if 'mds_confirm_count' in metrics:
        try:
            cnt = int(metrics['mds_confirm_count'])
            bot_state['mds_confirm_count'] = cnt
            try:
                bot = get_trading_bot()
                if hasattr(bot, 'mds_runner') and bot.mds_runner:
                    setattr(bot.mds_runner, '_confirm_count', cnt)
                    logger.info(f"[MDS] Runner confirm_count set to {cnt} from external metrics")
            except Exception:
                pass
        except Exception:
            logger.warning('[MDS] Invalid mds_confirm_count')

    logger.info(f"[MDS] Metrics updated: {updated}")
