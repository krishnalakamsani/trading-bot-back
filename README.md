# ScoreEngine (MDS) Trading Bot 🤖

An automated options trading bot for NSE indices (NIFTY, BANKNIFTY, SENSEX, FINNIFTY) using a ScoreEngine (MDS) strategy and the Dhan Trading API. Paper and live trading modes with a real-time dashboard.

---

## 🚀 Quick Features

✅ **ScoreEngine (MDS) Strategy** - Multi-candle confirmed entries based on score, slope, and slow momentum  
...
- Watch **Market Data** section for MDS signals
...
### ScoreEngine (MDS) Strategy
- **Decision basis**: Uses a numeric `score`, `slope`, and `slow_mom` (slow momentum) from a ScoreEngine (MDS).
- **Entries**: Multi-candle confirmation required (`mds_confirm_needed`) — entries occur only when the runner confirms direction, score, and slope thresholds.
- **Direction**: `CE` → buy calls, `PE` → buy puts; a neutral band (`NONE`) blocks entries.
- **Exit Conditions** (priority order):
  1. MDS-defined reversal or neutral conditions (slow momentum confirm)
  2. Max Loss Per Trade exceeded ⚠️
  3. Target Points hit ✓
  4. Trailing Stop Loss hit
  5. Daily Max Loss triggered
### Risk Management
...
- ScoreEngine (MDS) is NOT a guaranteed winning strategy
